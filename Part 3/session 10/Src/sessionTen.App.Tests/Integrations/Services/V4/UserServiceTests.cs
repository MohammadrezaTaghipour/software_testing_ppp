...
namespace sessionTen.App.Tests.Integrations.Services.V4;

public class UserServiceTests(IntegrationFixture fixture) : IClassFixture<IntegrationFixture>
{
    [Fact]
    public async Task Changing_email_from_non_corporate_to_corporate()
    {
        //Arrange
        var dbContext = await fixture.CreateContext();
    ...
    [Fact]
    public async Task Changing_email_from_corporate_to_nonCorporate()
    {
        //Arrange
        var dbContext = await fixture.CreateContext();
        await dbContext.Database.BeginTransactionAsync();
        ...
        var company = new Company(1, "edsub.ir", 1);
        dbContext.Companies.Add(company);

        var user = new User(1, "user@edsub.com", company);
        dbContext.Users.Add(user);
        ...
        var userActual = await dbContext.Users.FirstAsync(a => a.Id == user.Id);
        userActual.Email.Should().Be(request.NewEmail);
        userActual.Type.Should().Be(UserType.Customer);


        var companyActual = await dbContext.Companies.FirstAsync(a => a.Id == company.Id);
        companyActual.NumberOfEmployees.Should().Be(0);
